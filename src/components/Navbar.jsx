...
import ThemeToggleBtn1 from './ThemeToggleBtn1'
...
const Navbar = ({theme, setTheme}) => {
    ...
    return (
        <div className='flex justify-between items-center px-4 sm:px-12 lg:px-24 xl:px-40 py-4 sticky top-0 z-20 backdrop-blur-xl font-medium bg-white/50 dark:bg-gray-900/70'>
            ...
            <img src={theme === 'light' ? assets.logo : assets.logo_dark} className='w-32 sm:w-40' alt='Agency.ai' />


            <div className={`text-gray-700 dark:text-white sm:text-sm 
                    ${!sidebarOpen ? 'max-sm:w-0 max-sm:overflow-hidden' : 'max-sm:w-60 max-sm:pl-10'} 
                    max-sm:fixed max-sm:inset-y-0 max-sm:right-0 max-sm:min-h-screen max-sm:flex max-sm:flex-col 
            ...
            <div className='flex items-center gap-2 sm:gap-4'>
                ...
                <ThemeToggleBtn1 theme={theme} setTheme={setTheme} />


                <img src={theme === 'dark' ? assets.menu_icon_dark : assets.menu_icon} alt="" onClick={() => setSidebarOpen(true)} className='w-8 sm:hidden' />
                <a href="contact-us" className='text-sm max-sm:hidden flex item-center gap-2 bg-primary text-white
